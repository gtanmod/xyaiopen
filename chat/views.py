...
from .models import Conversation, Message, ChatSetting, AIModel, GlobalSetting
...
def chat(request, id=None):
    ...
    if chat_settings.exists():
        ...
        try:
            ...
            default_ai_prompt = global_setting.default_ai_prompt if global_setting else "你是一位专业、有帮助的人工智能助手，始终提供准确、有用的信息。"
        except Exception as e:
            logger.error(f"获取全局默认AI提示词时出错: {str(e)}")
...
def send_message(request):
    ...
    try:
        ...
        if user_profile.credits < estimated_total_cost:
            return JsonResponse({'error': '积分不足，请充值'}, status=402)
        ...
        with transaction.atomic():
            ...
            try:
                # 优先使用全局系统提示词
                global_setting = GlobalSetting.get_instance()
                if global_setting.system_prompt:
                    messages_for_api.insert(0, {
                        "role": "system",
                        "content": global_setting.system_prompt
                    })
                    logger.info(f"添加全局系统提示: {global_setting.system_prompt}")
                # 如果没有全局系统提示词，尝试使用模型的系统消息
                elif ai_model and hasattr(ai_model, 'system_message') and ai_model.system_message:
                    messages_for_api.insert(0, {
                        "role": "system",
                        "content": ai_model.system_message
                    })
                    logger.info(f"添加模型系统提示: {ai_model.system_message}")
            except Exception as e:
            ...
            if model_type == 'image':
                if 'data' in response and len(response['data']) > 0:
                    ...
                    if image_url:
                        ...
                        assistant_message = image_content
                        ...
                        CreditRecord.objects.create(
                            ...
                            record_type='usage',
                            description=f"使用{ai_model.display_name}生成图片",
                            message=user_msg,
                            balance=user_profile.credits
                        )
                        
                        # 修改这里：返回JsonResponse而不是使用yield
                        return JsonResponse(response_data)
                    else:
                        error_message = "图像生成失败，请稍后再试。"
                        return JsonResponse({'error': error_message}, status=500)
                else:
                    return JsonResponse({'error': '图像生成响应格式错误'}, status=500)
            else:
                # 处理聊天模型的标准响应格式
...
def stream_message(request):
    ...
    try:
        ...
        # 添加详细日志，记录请求内容
        logger.info(f"收到stream_message请求: conversation_id={conversation_id}, selected_model={selected_model}")
        ...
        try:
            ...
            return JsonResponse({'error': f'获取对话失败: {str(e)}'}, status=400)
        ...
        try:
            ...
            return JsonResponse({'error': f'获取用户设置失败: {str(e)}'}, status=500)
        ...
        try:
            # 确保获取正确的模型ID
            model_id_to_use = selected_model  # 优先使用选择的模型
            if not model_id_to_use:
                if isinstance(conversation.model, str):
                    model_id_to_use = conversation.model
                else:
                    # 假设是模型对象
                    try:
                        model_id_to_use = conversation.model.model_id
                    except:
                        model_id_to_use = "gpt-4o-all"  # 默认模型
            
            logger.info(f"使用模型ID: {model_id_to_use}")
            ai_model = AIModel.objects.get(model_id=model_id_to_use)
            logger.info(f"成功获取模型实例: {ai_model.model_id}, 支持图像: {ai_model.supports_image_analysis}")
        except Exception as e:
            logger.error(f"获取/确认模型实例失败: {str(e)}")
            # 设置默认值但不抛出错误
            model_id_to_use = "gpt-4o-all"  # 默认模型
            try:
                ai_model = AIModel.objects.get(model_id=model_id_to_use)
                ...
                logger.warning(f"无法获取默认模型，继续处理")
        ...
        # 优先使用前端传递的模型，如果没有则使用设置中的模型
        try:
            model_to_use = selected_model if selected_model else conversation.model.model_id
        ...
        def event_stream():
            ...
            # 在第一个事件中发送积分使用信息和对话标题信息
            yield f"data: {json.dumps({'credits_used': estimated_total_cost, 'credits_remaining': user_profile.credits, 'title': conversation.title})}\n\n"
            ...
            if model_type == 'image':
                try:
                    logger.info(f"处理图像生成请求: 模型={model_to_use}")
                    # 调用图像生成API
                    response = api_service.process_request(
                        model_id=model_to_use,
                        messages=messages_for_api,
                        temperature=chat_setting.temperature or 0.7,
                        max_tokens=chat_setting.max_tokens or 1500,
                        top_p=chat_setting.top_p or 1.0,
                        presence_penalty=chat_setting.presence_penalty or 0.0,
                        frequency_penalty=chat_setting.frequency_penalty or 0.0
                    )
                    
                    if 'error' in response:
                        error_msg = f"API响应错误: {response['error']}"
                        ...
                        if 'data' in response and len(response['data']) > 0:
                            ...
                            error_msg = "图像生成响应格式错误"
                            logger.error(f"{error_msg}: {response}")
                            yield f"data: {json.dumps({'error': error_msg})}\n\n"
                except Exception as e:
                    error_msg = f"处理图像请求时出错: {str(e)}" 
                    logger.error(error_msg)
                    yield f"data: {json.dumps({'error': error_msg})}\n\n"
                return
            
            # 使用流式响应处理聊天模型
            for chunk in api_service.stream_chat_completion(
                messages=messages_for_api,
                ...
                if not reasoning_data:
                    # 检查常规推理格式
                    current_reasoning = ChatAPIService.extract_reasoning(chunk, model_to_use)
                    if current_reasoning:
                        reasoning_data = current_reasoning
                        yield f"data: {json.dumps({'reasoning': reasoning_data})}\n\n"
                    
                    # 检查DeepSeek Reasoner特有的推理格式
                    if 'choices' in chunk and len(chunk['choices']) > 0:
                        choice = chunk['choices'][0]
                        # 检查消息中直接包含的推理
                        if 'message' in choice and 'reasoning' in choice['message']:
                            reasoning_data = choice['message']['reasoning']
                            yield f"data: {json.dumps({'reasoning': reasoning_data})}\n\n"
                        # 检查delta中的推理内容
                ...
                if 'choices' in chunk and len(chunk['choices']) > 0:
                    ...
                    # 处理message内容（非delta格式）
                    elif 'message' in choice:
                        message = choice['message']
                        if 'content' in message and message['content'] is not None:
                            content = message['content']
                            # 这里我们不累加，因为非流式格式会返回完整消息
                            yield f"data: {json.dumps({'content': content, 'is_full': True})}\n\n"
                            full_response = content  # 保存完整响应
                    
                    # 检查是否是最后一个消息块
                    if choice.get('finish_reason') is not None:
                        # 当生成完成时，如果还没有提取到推理数据，再尝试一次
                        if not reasoning_data:
                            # 构建完整响应对象以供提取
                            complete_response = {
                                'choices': [{'message': {'content': full_response}}]
                            }
                            reasoning_data = ChatAPIService.extract_reasoning(complete_response, model_to_use)
                            if reasoning_data:
                                yield f"data: {json.dumps({'reasoning': reasoning_data})}\n\n"
    ...
    if request.method == 'POST':
        try:
            ...
            if request.content_type == 'application/json':
                ...
                use_cot = data.get('use_cot') == 'on'
                use_functions = data.get('use_functions') == 'on'
            else:
                # 获取表单数据
            ...
            if chat_settings.exists():
                ...
                # 保存AI提示词（system_prompt不再从前端接收，只能通过Django后台设置）
                if ai_prompt is not None:
                    chat_setting.ai_prompt = ai_prompt
                ...
                if ai_prompt is not None:
                    ...
                    try:
                        global_setting = GlobalSetting.get_instance()
                        if global_setting.default_ai_prompt:
                            chat_setting.ai_prompt = global_setting.default_ai_prompt
                    except Exception as e:
                        logger.error(f"获取全局默认AI提示词时出错: {str(e)}")
